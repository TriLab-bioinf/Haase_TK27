...
run_enrichment_analysis <- function(gene_set, geneList, 
  ...
  if(dim(msig_h.gsea@result)[1] > 0){ 
    ...
    if(exists("p_ups_up")) print(p_ups_up)
    if(exists("p_ups_down")) print(p_ups_down)
    if(exists("p6")) print(p6)
    if(exists("p8")) print(p8)
    if(exists("p10")) print(p10)
    if(exists("p12")) print(p12)
    if(exists("p14")) print(p14)
    if(exists("p16")) print(p16)
  }
}
...
go_gsea <- function(dds_res, my_file, golevel = 3, 
  ...
  geneList.filtered <- rownames(dds_res)[dds_res$padj < qvalue]
  ...
  if(length(go.gsea.gs@result$NES) > 0){
    ...
    p2 <- dotplot(go.gsea.gs, showCategory=36, 
                  ...
                  color = "p.adjust", 
                  label_format = 100) + 
      ggtitle(paste0(my_file," dotplot for GSEA"))
...
PAPER_plot_gsea <- function(gsea_result, comparison_id, 
  ...
  if(length(gsea_result@result$NES) > 0){
    ...
    p_top_go_up <- get_gsea_clusters(my_gsea_result = gsea_result_bt_0, my_report_top_cutoff = REPORT_TOP_CUTOFF) # UP gene sets
    p_top_go_down <- get_gsea_clusters(my_gsea_result = gsea_result_lt_0, my_report_top_cutoff = REPORT_TOP_CUTOFF) # DOWN gene sets
